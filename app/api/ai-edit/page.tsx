...
export default function Home() {
  ...
  async function normalizeAiResultToThreeFour(src: string, backgroundColor = bg) {
    ...
    // Preserve the ENTIRE AI-generated half-body master image.
    // The AI result contains the full head, both arms and waist as safety area.
    // Never use a cover crop here, because that permanently removes those parts
    // and makes Zoom Out unable to reveal them later.
    //
    // Instead, fit the complete AI image inside a 3:4 master canvas and fill any
    // extra canvas area with the selected background colour. The 3:4 canvas itself
    // always fills the preview at 100%, while Zoom Out can safely reveal more of
    // the preserved half-body image without exposing a hard image edge.
        ...
        <section className="photoid-preview-card">
          ...
          {hasUploadedImage && (
            <div className="photoid-preview-actions">
              {!compareMode && <><button onClick={() => setZoom(Math.max(aiComposited ? 75 : 100, zoom - 5))}><ZoomOut /></button><b>{zoom}%</b>
