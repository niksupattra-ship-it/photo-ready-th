...
export async function POST(request:Request){
  try{
    ...
    const skinStrength=Math.max(0,Math.min(35,Number(input.get("skinStrength")||15)));
    ...
    const skinInstruction=operations.includes("skin-light")?` APPLY A VISIBLE LIGHTING RESULT. Natural-light preset: ${skinStyle}. The user selected ${skinStrength}% on a 0–35 interface scale; treat this as ${Math.round(skinStrength/35*100)}% of a professional lighting correction, not ${skinStrength} out of 100. ${skinStrength===0?"Make no lighting change.":skinStrength<=10?"Apply a gentle but visible exposure and white-balance correction.":skinStrength<=25?"Apply a clearly visible balanced exposure, white-balance, highlight, and shadow correction.":"Apply the strongest clearly visible yet realistic professional exposure and white-balance correction, with controlled highlights and opened shadows."} ${skinStyle==="สดใส"?"Create a clean, fresh, slightly brighter camera exposure while keeping the original complexion; never whiten the skin.":skinStyle==="สตูดิโอ"?"Create balanced soft neutral studio illumination with natural facial depth and dimensional shadows.":"Create neutral true-to-life camera exposure and accurate white balance with the original complexion unchanged."} Preserve pores, fine lines, blemishes, under-eye detail, facial contrast, and every real skin feature. The change must be noticeable in overall luminosity and balance, but must come only from photographic light correction—never skin smoothing, denoising, repainting, makeup, reshaping, or face regeneration.`:"";
