...
MATCH THE SELECTED HAIRSTYLE EXACTLY: reproduce image 2's defining hairstyle, including the same part location and direction, fringe or exposed forehead, crown height, side volume, temple shape, ear coverage, strand direction, length, ends, layers, curl or straightness, and front silhouette. Adapt only its fit to the existing skull in image 1. Do not invent a different generic hairstyle and do not copy the model's face, head proportions, clothing, pose, or background from image 2.

HAIR-ONLY EDIT MASK: the only region permitted to change is the existing hair plus the minimal immediately adjacent area required for new fine edge strands. Treat the face, forehead skin, eyebrows, eyes, nose, lips, jaw, ears, neck, shoulders, shirt, collar, tie, jacket, lapels, torso, background, and all non-hair pixels in image 1 as protected locked pixels. Copy those protected areas from image 1 unchanged. Do not redraw, reconstruct, retouch, beautify, smooth, relight, recolor, sharpen, or regenerate them. The identity, real skin texture, expression, outfit design, fabric texture, seams, buttons, and colors must remain identical to image 1.

ABSOLUTE GEOMETRY LOCK: preserve the exact canvas dimensions, crop, framing, field of view, perspective, head size, face coordinates, body scale, shoulder coordinates, margins, and background of image 1. No crop, zoom, resize, recenter, rotation, extension, or recomposition.
