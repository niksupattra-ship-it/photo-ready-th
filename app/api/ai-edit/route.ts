...
10) only a flat chroma-key background surrounds the person.`;
    ...
    const officialPersonOnlyPrompt=`OFFICIAL GOVERNMENT TEMPLATE MODE — PERSON ONLY, SINGLE AI CALL.

IMAGE ROLES:
- Image 1 = the ONLY identity/face/skin/head reference.
- Image 2 = the EXACT REAL government-uniform template. Use it ONLY to understand the fixed collar center, shoulder width, head-to-shoulder proportion, and intended camera scale. DO NOT render, redraw, imitate, recreate, redesign, recolor, simplify, or output any government-uniform pixels from image 2.
${useHairstyleReference?"- Image 3 = the EXACT selected hairstyle reference. It is authoritative for the hairstyle design.":""}

OUTPUT CONTENT — CRITICAL:
Return ONLY the same real person's COMPLETE HEAD + SELECTED/ORIGINAL HAIR + EARS + NATURAL NECK on one perfectly flat solid #FF00FF chroma background.
DO NOT output shoulders, civilian shirt, blouse, chest, torso, jacket, government uniform, epaulettes, tie, insignia, ribbon bars, buttons, sleeves, or any clothing.
Below the natural base of the neck there must be only flat #FF00FF background.
The website will place this person layer BEHIND the exact real PNG uniform template after the AI call. Therefore any clothing generated by AI would be wrong.

IDENTITY LOCK — SAME STANDARD AS JOB-APPLICATION MODE:
Preserve the exact identity from image 1: facial width/height, forehead, temples, cheeks, eye size/spacing, eyelids, eyebrows, nose, nostrils, philtrum, lips, smile, chin, jaw, asymmetry, ears, age, complexion, pores, blemishes, fine lines, under-eye detail, makeup, highlights, shadows, and natural camera texture.
Do not beautify, smooth, whiten, reshape, regenerate, average, or reinterpret the face.
The face may only move and uniformly scale together with the whole head/hair unit for fitting.

HEAD / NECK FIT TO THE REAL TEMPLATE:
Read image 2 only as fixed geometry. The template itself will be composited later unchanged.
Use a naturally smaller formal-portrait head scale: approximately 15% smaller than a typical close headshot, while preserving all internal facial proportions.
Keep the whole head/hair unit centered above image 2's collar opening.
Create a natural adult neck centered below the unchanged jaw, about 0.33–0.50 of head length where anatomically appropriate.
The neck must be continuous, symmetrical enough to look natural, and must NOT flare into shoulder-shaped skin wedges.
Do not include original clothing or original shoulders from image 1.

HAIRSTYLE:
${useHairstyleReference?`Image 3 is the exact selected hairstyle (${hairstyle}). Replace the original hairstyle with THIS selected design, not a similar or generic formal hairstyle. Match parting, bangs/front shape, side silhouette, crown volume, tied/untied state, length, layers, exposed-ear pattern, and visible endpoints. If the selected style is short/tied/updo, remove every contradictory long-hair remainder. Transfer HAIR ONLY; never copy image-3 face, skin, ears, neck, accessories, lighting, or background.`:`Keep image 1's original hairstyle exactly, including length, parting, volume and natural hairline.`}
Preserve natural fine hair strands and a clean soft outer edge. No blue/cyan/pink/purple/magenta fringe, no halo, no hard cutout outline, no rectangular residue, no wig-like edge.

LIGHT / SKIN:
${skinInstruction || "Keep image 1's natural skin tone, lighting and texture unchanged."}
${chosen.join(" ")}
${volumeInstruction}

BACKGROUND:
Use ONLY one perfectly flat #FF00FF background surrounding the head/hair/neck.
No blue background, no gradient, no texture, no shadows, no vignette.
Do not let #FF00FF contaminate hair or skin edges.

FINAL CHECK:
1) same person as image 1;
2) exact selected hairstyle when requested;
3) full head/hair visible with clean edges;
4) natural centered neck;
5) absolutely NO clothing/shoulders/torso/uniform in the AI output;
6) flat #FF00FF everywhere outside head/hair/neck.`;

    const body=new FormData();
    body.append("model","gpt-image-1.5");body.append("image[]",image,image.name||"portrait.png");body.append("image[]",outfit,outfit.name||"outfit-reference.png");if(useHairstyleReference)body.append("image[]",hairstyleRef,hairstyleRef.name||"hairstyle-reference.png");body.append("prompt",isOfficialTemplate?officialPersonOnlyPrompt:prompt+lockedFramingPrompt);body.append("input_fidelity","high");body.append("quality","high");body.append("output_format","png");body.append("size","1024x1536");body.append("n","1");
