...
export async function POST(request: Request) {
  try {
    ...
    const prompt = hairstyleOnly
      ...
      : outfitChange
        ? `Create one finished formal front-facing ID portrait as a transparent-background PNG. Image 1 is the absolute source of truth for the person. Image 2 is clothing reference only: dress the person in ${outfitLabel}, matching its collar, lapels, fabric, construction, silhouette, and proportions. Remove the source background completely in this same edit and make every background pixel fully transparent. Preserve soft, clean, anti-aliased edges and realistic individual hair strands; leave no gray background, halo, color spill, jagged edge, or leftover background pixel. ${chosen.join(" ")}${volumeInstruction} ABSOLUTE FACE, SKIN, AND LIGHTING LOCK: preserve the entire face and every visible skin area from image 1, including identity, expression, pores, blemishes, fine lines, complexion, exposure, highlights, shadows, color temperature, white balance, and illumination. Never retouch, smooth, blur, airbrush, brighten, whiten, recolor, relight, denoise, add makeup, beautify, repaint, or regenerate face or skin. Preserve the hair and head size. Change only the clothing, the minimum necessary collar/neck boundary, and the background alpha channel. COMPOSITION LOCK: show the complete left and right shoulder lines and both upper arms fully inside the frame with a small, even safe margin on both sides. No sleeve, arm, shoulder, lapel, or garment edge may touch or be cut by the left or right canvas edge. Fit the complete person-and-outfit composition into the existing canvas by applying only one uniform proportional scale to the whole subject when necessary; never stretch, squeeze, widen, narrow, or independently resize the head, face, neck, shoulders, arms, or torso. Keep the subject centered. Keep the head, neck, shoulders, outfit, and torso at one uniform natural adult photographic scale. Never make the head oversized or the body narrow or miniature. Preserve the canvas and camera perspective. The outfit must show realistic woven fabric, seams, folds, depth, and non-uniform camera highlights—never plastic, illustrated, or synthetic. The result must look like the same real person genuinely photographed wearing the selected outfit.`
