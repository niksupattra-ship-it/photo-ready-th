...
export async function POST(request: Request) {
  try {
    ...
    const prompt = hairstyleOnly
      ? `HAIRSTYLE-ONLY EDIT. Image 1 is the finished portrait that must be preserved. Image 2 is a visual hairstyle reference for ${requestedHairstyle}; use ONLY its hair design and ignore its model entirely. Replace the existing hairstyle in image 1 with the clearly recognizable hairstyle from image 2. The result must visibly match image 2 in parting, fringe or no fringe, exposed forehead, outer silhouette, length, layers, direction, volume, tuck behind ears, tied or loose arrangement, and strand flow. Do not keep the original hairstyle and do not invent a generic substitute. Fit that hair design naturally and proportionally around the unchanged skull, face shape, forehead, and ears of the person in image 1. Permit edits only within the existing hair region and the smallest immediately adjacent area required by the new hair silhouette or newly revealed forehead; reconstruct newly revealed forehead using the person's own adjacent skin appearance from image 1. Apply a professional natural-black tone at 50% intensity while retaining realistic dark-brown and charcoal variation, individual strands, translucent edge hairs, natural roots, gravity, and soft camera highlights. IDENTITY AND PORTRAIT LOCK: the person must remain unmistakably and exactly the person from image 1. Preserve their facial geometry, expression, eyes, eyebrows, eyelashes, nose, lips, jaw, complexion, pores, marks, makeup, ears, neck, shoulders, arms, body, selected outfit, fabric, lapels, collar, pose, lighting, exposure, shadows, white balance, background, canvas size, crop, perspective, position, and scale. Never copy the face, skin, eyebrows, ears, neck, clothing, body, lighting, or background of the model in image 2. Never overlay or blend the model from image 2. Do not retouch, beautify, smooth, brighten, recolor, relight, move, resize, or recompose the person. Output image 1 with one obvious change only: the selected hairstyle.`
      : outfitChange
