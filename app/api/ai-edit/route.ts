...
IMAGE 2 = the EXACT REAL GOVERNMENT-UNIFORM TEMPLATE. It is a GEOMETRY REFERENCE ONLY. Measure its real neck opening, inner collar contact points, collar height and center line. Do NOT recreate the uniform.
${useHairstyleReference?`IMAGE 3 = the EXACT selected hairstyle (${hairstyle}). It is a HAIR reference only.`:""}

CRITICAL FIT TARGET:
- keep the COMPLETE hairstyle inside the image with generous clear space above the highest hair point; NEVER let the top edge touch or cut the hair;
- preserve the whole real face identity exactly;
- generate a FULL continuous neck from jaw to below the collar-contact depth;
- use IMAGE 2 to make the lower neck width naturally fill the real collar opening with no blue/magenta/background gap;
- the neck must taper anatomically from the jaw and widen only as needed toward the collar;
- do not make a rectangular neck, floating head, detached jaw, or visible cut line;
- the website will place the untouched real template over this person layer and may locally open/close ONLY the inner collar contact edge. Extend natural skin safely BEHIND that collar area.
...
Keep at least about 10% of the square canvas as clear background above the HIGHEST visible hair point and clear margin beside the complete hairstyle.
Never crop or truncate hair at the top, sides, back or endpoints. If the source crop is tight, reconstruct only the missing HAIR continuation naturally while keeping the face/head anatomy unchanged. The topmost hair contour must be complete and rounded/natural, never flat or horizontal.
...
Keep the neck centered, smooth, natural and long enough to extend behind the real collar in IMAGE 2.
At the lower neck, match the measured inner-collar opening width from IMAGE 2 so skin fully occupies the opening before the real collar is composited on top. Blend jaw-to-neck skin continuously with identical source complexion, exposure and texture.
