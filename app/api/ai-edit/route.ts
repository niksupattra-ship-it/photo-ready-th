...
export async function POST(request:Request){
  try{
    ...
    const skinInstruction=operations.includes("skin-light")?` Natural-light preset: ${skinStyle}. Adjustment strength: ${skinStrength} out of 100, intentionally subtle and capped. ${skinStyle==="สดใส"?"Lift only the overall exposure very slightly while keeping the original complexion; never whiten skin.":skinStyle==="สตูดิโอ"?"Balance the whole photograph like soft neutral studio lighting while retaining natural facial shadows and depth.":"Use neutral true-to-life exposure and white balance with the original complexion unchanged."} Preserve pores and all real skin texture at every strength. Correct only whole-photo lighting and temporary color cast, never smooth or repaint skin.`:"";
