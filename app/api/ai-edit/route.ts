...
export async function POST(request:Request){
  try{
    ...
    const prompt=`Create one finished formal front-facing ID portrait. Image 1 is the person and is the absolute identity reference. Image 2 is clothing reference only: dress the person in ${outfitLabel} matching its collar, lapels, fabric, and proportions. Use a clean solid background color ${background}. ${chosen.join(" ")}${volumeInstruction} Preserve the face from image 1 exactly: identical facial structure, eyes, eyebrows, nose, lips, expression, age, skin texture, and recognizable features. Do not beautify or replace the face. Center the head and torso, show the full head and shoulders, and connect the neck naturally to the selected clothing. Preserve fine hair edges. Return one seamless finished professional studio portrait, not a layered mockup.`;
