...
const hairstyleOptions = ["original", ...Array.from({ length: 29 }, (_, index) => `hair-${String(index + 1).padStart(2, "0")}`)].map((id) => ({
  id,
...
export default function Home() {
  ...
  function pickFile(e:React.ChangeEvent<HTMLInputElement>){ const f=e.target.files?.[0]; if(f){setOriginal(URL.createObjectURL(f));setCutout(null);setHairstyle("original");hairstyleBase.current=null;setProcessMessage("");} }
  ...
  async function aiEdit(hairstyleId = hairstyle){
    ...
    try{
      const selectedHairstyle=hairstyleOptions.find(item=>item.id===hairstyleId);
      const hairOnly=hairstyleId!=="original";
      const sourceUrl=hairOnly?(hairstyleBase.current||original):original;
      const source=await fetch(sourceUrl);const blob=await source.blob();
      ...
      if(hairOnly&&selectedHairstyle?.image){const ref=await fetch(selectedHairstyle.image);form.append("hairstyleRef",await ref.blob(),`${hairstyleId}.png`);form.append("hairstyle",selectedHairstyle.label);form.append("operations",JSON.stringify(["hairstyle"]));}
  ...
  async function selectHairstyle(id:string){
    ...
    setHairstyle(id);
    if(id==="original"){
      if(hairstyleBase.current){setOriginal(hairstyleBase.current);setCutout(null);setProcessMessage("กลับมาใช้ทรงผมเดิมแล้ว");}
      return;
    }
